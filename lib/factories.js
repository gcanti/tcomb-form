...
config.kinds = {
  irriducible: function (type, opts) {
    ...
    return config.irriducibles.hasOwnProperty(name) ?
      ...
      textboxFactory; // fallback on textbox
  },
  enums:    function () { return selectFactory; },
  struct:   function () { return structFactory; },
  list:     function () { return listFactory; },
...
function textboxFactory(opts, ctx) {
  ...
  return React.createClass({
    ...
    getValue: getValueFactory(report.type),
...
function selectFactory(opts, ctx) {
  ...
  var emptyOption = opts.emptyOption || {value: '', text: '-'};
