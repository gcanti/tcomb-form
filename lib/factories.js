...
function tuple(opts, ctx) {
  ...
  var components = types.map(function (coordinateType, i) {
    var coordinateOpts = opts.coordinates ? opts.coordinates[i] : {};
    var factory = getFactory(coordinateType, coordinateOpts);
    return factory(coordinateOpts, new Context({
      auto:       auto,
      config:     config,
      i18n:       i18n,
      label:      '#' + (i + 1),
      name:       ctx.name ? ctx.name + '[' + i + ']' : i + '',
      report:     new getReport(coordinateType),
      templates:  templates,
      value:      value[i]
    }));
  });
  ...
  return React.createClass({
    ...
    onCoordinateChange: function (coordinateIndex, coordinateValue) {
      var value = this.state.value.slice();
      value[coordinateIndex] = coordinateValue;
      this.onChange(value);
    },
    ...
    render: function () {
      ...
      var coordinates = components.map(function (component, i) {
