...
api.Select.renderers.select = function select(type, options, ctx) {
  ...
  if (!opts) {
    opts = getOptionsOfEnum(report.last);
  }
  ...
  var List = React.createClass({
    ...
    render: function () {
      ...
      var rows = this.state.value.map(function getRow(value, i) {
        ...
        var buttons = [];
        if (!options.disabledRemove) { buttons.push({ label: bundle.remove, click: this.remove.bind(this, i) }); }
        if (!options.disableOrder) { buttons.push({ label: bundle.up, click: this.up.bind(this, i) }); }
        if (!options.disableOrder) { buttons.push({ label: bundle.down, click: this.down.bind(this, i) }); }
