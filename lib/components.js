...
function getComponent(_x, _x2) {
  ...
  _function: while (_again) {
    ...
    switch (type.meta.kind) {
      case 'irreducible':
        return type === _tcombValidation2['default'].Boolean ? Checkbox : type === _tcombValidation2['default'].Date ? Datetime : Textbox;
      case 'struct':
        return Struct;
      case 'list':
        return List;
...
var Datetime = (function (_Component5) {
  ...
  _createClass(Datetime, null, [{
    ...
    value: {
      format: function format(value) {
        return _tcombValidation2['default'].Array.is(value) ? value : _tcombValidation2['default'].Date.is(value) ? [value.getFullYear(), value.getMonth(), value.getDate()].map(String) : [null, null, null];
      },
      parse: function parse(value) {
        value = value.map(parseNumber);
        return value.every(_tcombValidation2['default'].Number.is) ? new Date(value[0], value[1], value[2]) : value.every(Nil.is) ? null : value;
      }
    },
    enumerable: true
  }]);
