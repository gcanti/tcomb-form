...
function recurse(type, options, ctx) {
  ...
  // retrieves type meta infos to help
  // choosing the correct input and
  // building labels and placeholders
  ...
  // [extension point]
  var Api = options.api || analyze(type);
...
function getInnerType(type) {
  ...
  switch (kind) {
    case 'subtype' :
    case 'maybe' :
      return getInnerType(type.meta.type);
    default :
      return type;
  }
}
...
api.Textbox.prototype.toDSL = function (report, ctx) {
  ...
  if (!transformer && report.innerType === t.Num) {
...
api.Struct.prototype.toDSL = function (report, ctx) {
  ...
  var rows = order.map(function (prop) {
    ...
    return recurse(props[prop], options, new Context({
...
api.List.prototype.toDSL = function (report, ctx) {
  ...
  function getRow(value, i) {
    options.value = value;
    return recurse(itemType, options, new Context({
