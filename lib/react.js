...
// utils
var getTypeReport = require('./util/getTypeReport');
...
function textboxFactory(type, opts, ctx) {
  ...
  if (!label && !placeholder && ctx.auto !== 'none') {
    placeholder = ctx.defaultLabel;
    if (report.isMaybe) {
      placeholder += ctx.i18n.optional;
    }
  }

  var name = opts.name || ctx.getNameAttribute();
...
function structFactory(type, opts, ctx) {
  ...
  for (var k in props) {
    if (props.hasOwnProperty(k)) {
      ...
      var Component = factory(props[k], fields[k], new api.Context({
        i18n: i18n,
        auto: auto,
...
function listFactory(type, opts, ctx) {
  ...
  var getComponent = function getComponent(value, i) {
    return itemFactory(itemType, opts.item, new api.Context({
      i18n: i18n,
      auto: auto,
...
api.Context.prototype.getDisplayName = function () {
  return 'tcomb-form ' + (this.getNameAttribute() || 'root') + ' form / field';
};
