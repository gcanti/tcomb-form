...
var getNameAttribute = require('./util/getNameAttribute');
...
function structFactory(type, opts, ctx) {
  ...
  for (var k in props) {
    if (props.hasOwnProperty(k)) {
      ...
      var factory = getFactory(props[k], field);
  ...
  var List = React.createClass({
    ...
    render: function () {
      ...
      var rows = this.state.value.map(function getRow(value, i) {
        ...
        var itemFactory = getFactory(report.last, opts.item);
        var Component = itemFactory(report.last, opts.item || {}, {
          bundle: ctx.bundle,
          auto: auto,
