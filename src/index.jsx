  ...
  var factories = order.map(function (name) {
    ...
    if (auto === 'label') {
      if (Input === form) {
        ...
        o.label = o.label || <span>{humanize(name)}<small className="text-muted">{optional}</small></span>;
      }
      if (Input === select) {
      ...
      if (Input === form) {
        ...
        o.label = <span>{humanize(name)}<small className="text-muted">{optional}</small></span>;
